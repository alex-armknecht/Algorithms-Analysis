...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        ...
        int[] availableMoves = getMoves();
        for (int row = 0; row < this.state.length; row++) {
            for (int col = 0; col < this.state.length; col++) {
                if (state[row][col] == 0) {
                    for (int i = 0; i < availableMoves.length; i++ ) {
                        T3Action availableAction = new T3Action(col, row, availableMoves[i]);
                        T3State availableState = getNextState(availableAction);
                        transitions.put(availableAction, availableState);
                    }
                }
            }
        }
